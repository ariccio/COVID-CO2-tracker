module Export
  class BaseService
    ...
    def measurements_query(filters = @filters)
      Export::QueryBuilder.new.build(filters: filters)
    end
    ...
    def validate_filters!
      ...
      if @filters[:above_ppm] && @filters[:above_ppm].to_i < 0
        raise ExportError, "Invalid CO2 threshold: must be non-negative"
      end
      
      if @filters[:below_ppm] && @filters[:below_ppm].to_i < 0
        raise ExportError, "Invalid CO2 threshold: must be non-negative"
      end
      
      if @filters[:above_ppm] && @filters[:below_ppm]
        if @filters[:above_ppm].to_i >= @filters[:below_ppm].to_i
