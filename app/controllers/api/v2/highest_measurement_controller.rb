class Api::V2::HighestMeasurementController < ApplicationController
  def index
    ...
    @ten_sublocations = SubLocation.joins(:measurement).order(co2ppm: :desc).select(:id, :description, :place_id).first(10)
    @ten_places = Place.joins(sub_location: :measurement).order(co2ppm: :desc).select(:id, :google_place_id).first(10)
